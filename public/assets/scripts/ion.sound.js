...
;(function (window, navigator, $, undefined) {
    ...
    if (!settings.supported && is_iOS) {
        settings.supported = ["mp3", "aac"];
    } else if (!settings.supported) {
        settings.supported = ["mp3", "ogg", "aac", "wav"];
    }
    ...
    Stream.prototype = {
        ...
        play: function (options) {
            ...
            // If slow is true, than playback rate will be less than normal speed
            if (options.slow) {
                this.source.playbackRate.value = 0.88;
            }
