    ...
    .factory("SatellizerShared", [
      ...
      function ($q, $window, $log, config, storage) {
        ...
        var tokenName = config.tokenPrefix
          ? [config.tokenPrefix, config.tokenName].join("_")
          : config.tokenName;
        ...
        Shared.setToken = function (response) {
          ...
          if (accessToken) {
            if (
              angular.isObject(accessToken) &&
              angular.isObject(accessToken.data)
            ) {
          ...
          if (!token && response) {
            ...
            token = tokenRootData
              ? tokenRootData[config.tokenName]
              : response.data && response.data[config.tokenName];
          }

          if (!token) {
            var tokenPath = config.tokenRoot
    ...
    .factory("SatellizerOauth", [
      ...
      function ($q, $http, config, utils, shared, Oauth1, Oauth2) {
        ...
        Oauth.authenticate = function (name, userData) {
          var provider =
            config.providers[name].oauthType === "1.0"
              ? new Oauth1()
              : new Oauth2();
        ...
        Oauth.unlink = function (provider, opts) {
          ...
          opts.url = opts.url
            ? opts.url
            : utils.joinUrl(config.baseUrl, config.unlinkUrl);
    ...
    .factory("SatellizerLocal", [
      ...
      function ($http, utils, shared, config) {
        ...
        Local.login = function (user, opts) {
          ...
          opts.url = opts.url
            ? opts.url
            : utils.joinUrl(config.baseUrl, config.loginUrl);
        ...
        Local.signup = function (user, opts) {
          ...
          opts.url = opts.url
            ? opts.url
            : utils.joinUrl(config.baseUrl, config.signupUrl);
          opts.data = user || opts.data;
          opts.method = opts.method || "POST";
          opts.withCredentials = opts.withCredentials || config.withCredentials;
      ...
      function ($q, $http, $window, $timeout, popup, utils, config, storage) {
        return function () {
          ...
          Oauth2.open = function (options, userData) {
            ...
            openPopup.then(
              function (oauthData) {
                ...
                if (
                  oauthData.state &&
                  oauthData.state !== storage.get(stateName)
                ) {
                  return defer.reject(
          ...
          Oauth2.exchangeForToken = function (oauthData, userData) {
            ...
            var exchangeForTokenUrl = config.baseUrl
              ? utils.joinUrl(config.baseUrl, defaults.url)
              : defaults.url;
          ...
          Oauth2.buildQueryString = function () {
            ...
            angular.forEach(urlParamsCategories, function (paramsCategory) {
              angular.forEach(defaults[paramsCategory], function (paramName) {
                ...
                var paramValue = angular.isFunction(defaults[paramName])
                  ? defaults[paramName]()
                  : defaults[camelizedName];
                ...
                if (paramName === "scope" && Array.isArray(paramValue)) {
                  ...
                  if (defaults.scopePrefix) {
                    paramValue = [defaults.scopePrefix, paramValue].join(
                      defaults.scopeDelimiter
                    );
                  }
                }
    ...
    .factory("SatellizerPopup", [
      ...
      function ($q, $interval, $window, config, utils) {
        ...
        Popup.open = function (url, name, options) {
          ...
          var stringifiedOptions = Popup.stringifyOptions(
            Popup.prepareOptions(options)
          );
          var UA = $window.navigator.userAgent;
          var windowName =
            window.cordova || UA.indexOf("CriOS") > -1 ? "_blank" : name;
        ...
        Popup.pollPopup = function (redirectUri) {
          ...
          var polling = $interval(function () {
            ...
            try {
              var popupWindowPath = utils.getFullUrlPath(
                Popup.popupWindow.location
              );
              ...
              if (popupWindowPath === redirectUriPath) {
                ...
                if (
                  ...
                  var queryParams = Popup.popupWindow.location.search
                    .substring(1)
                    .replace(/\/$/, "");
                  var hashParams = Popup.popupWindow.location.hash
                    .substring(1)
                    .replace(/[\/$]/, "");
                  var hash = utils.parseQueryString(hashParams);
                  var qs = utils.parseQueryString(queryParams);
    ...
    .factory("SatellizerStorage", [
      ...
      function ($window, $log, config) {
        ...
        var isStorageAvailable = (function () {
          try {
            var supported =
              config.storageType in $window &&
              $window[config.storageType] !== null;
        ...
        return {
          get: function (key) {
            return isStorageAvailable
              ? $window[config.storageType].getItem(key)
              : store[key];
          },
          set: function (key, value) {
            return isStorageAvailable
              ? $window[config.storageType].setItem(key, value)
              : (store[key] = value);
          },
          remove: function (key) {
            return isStorageAvailable
