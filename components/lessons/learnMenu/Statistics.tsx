...
const Statistics: FC<Props> = ({ courseId, token }) => {
  ...
  useEffect(() => {
    const fetchGrammarData = async () => {
      try {
        const response = await getStatistics({ courseId, token })
  ...
  const CourseProgress = () => (
    <div className={style.progressWrapper}>
      ...
      <div>
        ...
        <span className={style.tasksLeft}>{statisticsData?.leftScore}</span>
      </div>
    </div>
  )
        ...
        {statisticsData && (
          <div className={style.progressContainer}>
            <span
              ...
              style={{
                width:
                  (statisticsData.totalTimeSpent / statisticsData.timeLeft) *
                    100 +
                  '%',
              }}
            />
